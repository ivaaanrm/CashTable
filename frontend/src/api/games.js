...
export async function getGame(id) {
  const res = await authFetch(`${BASE}/games/${id}`)
...
export async function closeGame(id) {
  const res = await authFetch(`${BASE}/games/${id}/close`, { method: 'PATCH' })
  if (!res.ok) throw new Error('Error al cerrar la partida')
  return res.json()
}

export async function deleteGame(id) {
  const res = await authFetch(`${BASE}/games/${id}`, { method: 'DELETE' })
...
export async function getSettlement(id) {
  const res = await authFetch(`${BASE}/games/${id}/settlement`)
